...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 heating.py <heating_threshold> <cooling_threshold>")
        sys.exit(1)
    ...
    for outside_temp in sys.stdin:
        outside_temp = outside_temp.strip()
        ...
        try:
            outside_temp = float(outside_temp)  # Convert to float
        except ValueError:
            print(f"Invalid input, skipping: '{outside_temp}'")
            continue
